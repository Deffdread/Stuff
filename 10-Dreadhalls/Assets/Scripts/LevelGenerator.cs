	...
	void Start () {
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				else if (Random.Range(0,8) == 0 && holes > 0 && characterPlaced){ //Create hole
					
					CreateChildPrefab(floorPrefab, floorParent, x, -2, z);
					holes -= 1;
					holePlaced = true;
				}
				...
				if(holePlaced == true){
					...
					CreateChildPrefab(floorPrefab, floorParent, x, -1, z);
				}
				holePlaced = false;
