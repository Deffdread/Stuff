...
function LevelMaker.createMap(level)
    ...
    -- don't go above 3
    local highestTier = math.min(3, level % 5)

    -- highest color of the highest tier, no higher than 5
    local highestColor = math.min(5, 1 + math.floor(level / 5)) --1 = blue, lowest color
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            -- Locked or dud brick
            local special = math.random(1,10) --testing atm
            if special == 1 then
                special = true
            else
                special = false
            ...
            --overwrite any characteristics and make it the golden brick
